...
double
Perceptron::eval(uint64_t pc, std::vector<uint8_t> signature) const {
  double accum = 0.0;
...
void
Perceptron::train(uint64_t pc, std::vector<uint8_t> signature, bool correct) {
  for(size_t i = 0; i < signature.size(); i++) {
    if(correct) {
...
Classifier::Classifier(size_t actions, size_t features, int init_range, double eta) {
  this->actions.resize(actions, Perceptron(features, init_range, eta));
}

int
Classifier::eval(uint64_t pc, std::vector<uint8_t> signature) {
...
void
Classifier::train(uint64_t pc, std::vector<uint8_t> signature, int action) {
